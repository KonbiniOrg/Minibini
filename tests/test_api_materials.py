...
class MaterialAssignTaskApiTest(APITestCase):
    ...
    def setUp(self):
        ...
        self.scheme = ServicePrice.objects.create(
            name='S-masgn', algorithm=ServicePrice.FLAT_FEE,
        ...
        self.task_a = Task.objects.create(name='A', job=self.job, service_price=self.scheme)
        self.task_b = Task.objects.create(name='B', job=self.job, service_price=self.scheme)
        self.task_done = Task.objects.create(name='Done', job=self.job, status='complete', service_price=self.scheme)
        self.other_job = Job.objects.create(job_number='JOB-ASGN-2', contact=contact)
        self.other_task = Task.objects.create(name='Other', job=self.other_job, service_price=self.scheme)
