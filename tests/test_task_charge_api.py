...
from apps.jobs.models import ServicePrice, Task
...
class TaskBillingFieldsAPITest(TestCase):
    ...
    def setUp(self):
        ...
        self.task = Task.objects.create(name='Test Task', job=self.job, service_price=self.scheme)
    ...
    def test_task_serializer_has_no_charge_key(self):
        ...
        # service_price and active_modifiers are now top-level
    ...
    def test_task_serializer_includes_billing_top_level(self):
        ...
        self.task.service_price = self.scheme
        ...
        body = resp.json()
        self.assertNotIn('charge', body)
        self.assertEqual(body['service_price'], self.scheme.pk)
...
class TaskSerializerNoLegacyFieldsTest(BaseTestCase):
    ...
    def setUp(self):
        ...
        from django.contrib.auth.models import Permission
        from apps.jobs.models import ServicePrice, Job, Task
        ...
        self.task = Task.objects.create(job=self.job, name='T-tnf', service_price=self.scheme)
    ...
    def test_task_list_omits_legacy_fields(self):
        ...
        self.assertIn('service_price', first)
...
class TaskTimeFieldsTest(BaseTestCase):
    ...
    def setUp(self):
        ...
        from apps.core.models import AccountingCategory, User
        from django.contrib.auth.models import Permission
        from apps.jobs.models import ServicePrice, Job, Task, PlanTask, Blep
        ...
        self.elapsed_scheme = ServicePrice.objects.create(
        ...
        self.plan_task = PlanTask.objects.create(
            ...
            service_price=self.elapsed_scheme, est_qty=Decimal('4.0'),
        )
        self.elapsed_task = Task.objects.create(
            job=self.job, name='Cut', source_plan_task=self.plan_task,
            est_qty=Decimal('4.0'), service_price=self.elapsed_scheme,
        )
        ...
        self.flat_task = Task.objects.create(job=self.job, name='Setup', service_price=self.flat_scheme)
        Blep.objects.create(
...
class TaskTemplateSerializerNoACTest(BaseTestCase):
    ...
    def setUp(self):
        ...
        self.template = TaskTemplate.objects.create(
            template_name='T-tts', service_price=scheme,
