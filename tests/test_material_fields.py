...
class MaterialFieldsTest(TestCase):
    def setUp(self):
        ...
        self.scheme = ServicePrice.objects.create(
            name='S-mf', algorithm=ServicePrice.FLAT_FEE,
            rate=1, unit_label='ea', accounting_category=self.cat,
        )
        self.task = Task.objects.create(job=self.job, name='t', service_price=self.scheme)
...
class PlanMaterialFieldsTest(TestCase):
    def setUp(self):
        ...
        self.pmf_scheme = ServicePrice.objects.create(
            name='S-pmf', algorithm=ServicePrice.FLAT_FEE,
            rate=Decimal('1'), unit_label='ea',
            accounting_category=self.pmf_ac,
        )
        self.pt = PlanTask.objects.create(
            est_worksheet=self.ws, name='pt1',
            service_price=self.pmf_scheme, est_qty=Decimal('1'),
        )
...
class MaterialTaskSetNullTest(TestCase):
    ...
    def setUp(self):
        ...
        scheme = ServicePrice.objects.create(
            name='S-tsn', algorithm=ServicePrice.FLAT_FEE,
            rate=1, unit_label='ea', accounting_category=cat,
        )
        self.task = Task.objects.create(job=self.job, name='deletable', service_price=scheme)
