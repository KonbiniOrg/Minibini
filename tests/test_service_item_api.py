...
class ServiceItemAPITest(TestCase):
    def setUp(self):
        ...
        self.scheme = ServiceItem.objects.create(
            name='Hourly Labor', algorithm=ServiceItem.ELAPSED_TIME,
            rate=Decimal('45.00'), unit_label='hours',
            accounting_category=self.ac,
        )

    def test_list_requires_auth(self):
        resp = self.client.get('/api/service-items/')
    ...
    def test_update(self):
        ...
        resp = self.client.patch(
            f'/api/service-items/{self.scheme.pk}/',
    ...
    def test_delete(self):
        self.client.login(username='admin', password='testpass')
        resp = self.client.delete(f'/api/service-items/{self.scheme.pk}/')
        self.assertEqual(resp.status_code, 200)
        self.assertFalse(ServiceItem.objects.filter(pk=self.scheme.pk).exists())

    def test_retrieve(self):
        self.client.login(username='worker', password='testpass')
        resp = self.client.get(f'/api/service-items/{self.scheme.pk}/')
        self.assertEqual(resp.status_code, 200)
        self.assertEqual(resp.json()['name'], 'Hourly Labor')


class ServiceItemEditBlockTest(BaseTestCase):
    fixtures = []
...
class ServiceItemSupersedeEndpointTest(BaseTestCase):
    fixtures = []
    ...
    def test_supersede_creates_new_and_links_old(self):
        ...
        resp = self.client.post(
            f'/api/service-items/{old.pk}/supersede/',
            {
        ...
        self.assertEqual(resp.status_code, 201)
        new_id = resp.json()['service_item_id']
        old.refresh_from_db()
    ...
    def test_supersede_requires_can_manage_config(self):
        ...
        resp = self.client.post(f'/api/service-items/{old.pk}/supersede/', {})
        self.assertEqual(resp.status_code, 403)
...
class ServiceItemListFilterTest(BaseTestCase):
    fixtures = []
    ...
    def setUp(self):
        ...
        self.active = ServiceItem.objects.create(
            name='A-lf', algorithm='flat_fee', rate=Decimal('1'),
            unit_label='ea', accounting_category=self.ac,
        )
        self.old = ServiceItem.objects.create(
    ...
    def test_default_list_excludes_superseded(self):
        ...
        ids = [r['service_item_id'] for r in items]
    ...
    def test_include_superseded_returns_all(self):
        resp = self.client.get('/api/service-items/?include_superseded=true')
        body = resp.json()
        items = body.get('results', body)
        ids = [r['service_item_id'] for r in items]
    ...
    def test_only_superseded_returns_just_old(self):
        resp = self.client.get('/api/service-items/?only_superseded=true')
        body = resp.json()
        items = body.get('results', body)
        ids = [r['service_item_id'] for r in items]
        self.assertIn(self.old.pk, ids)
...
class ServiceItemSerializerExtraFieldsTest(BaseTestCase):
    fixtures = []
...
class ServiceItemSupersedeSameNameTest(BaseTestCase):
    """
    ...
    def test_supersede_with_same_name_as_old_succeeds(self):
        ...
        self.assertEqual(
            ServiceItem.objects.filter(name='SN-Hourly').count(), 1,
        )
        self.assertEqual(
            ServiceItem.objects.filter(name='SN-Hourly (v1)').count(), 1,
        )
...
class ServiceItemTaskApplicableFilterTest(TestCase):
    """GET /api/service-items/?task_applicable=true must exclude percentage services."""
