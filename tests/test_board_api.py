...
class TaskReorderEndpointTest(FixtureTestCase):
    def setUp(self):
        ...
        self.task1 = Task.objects.create(
            ...
            service_price_id=1, est_worker_time=timedelta(hours=1),
        )
    ...
    def test_assign_task_via_patch(self):
        unassigned_task = Task.objects.create(
            name='Unassigned', job=self.job, service_price_id=1,
...
class TaskAssignEndpointTest(FixtureTestCase):
    ...
    def test_assign_without_worker_time_prompts(self):
        ...
        task = Task.objects.create(
            name='No estimate', job=self.job, service_price_id=1,
        )
        response = self.client.post(
            f'/api/tasks/{task.pk}/assign/',
            data={'assignee': self.user.pk, 'worker_queue': 1},
    ...
    def test_assign_task_that_already_has_worker_time(self):
        ...
        task = Task.objects.create(
            name='Has estimate', job=self.job, service_price_id=1,
    ...
    def test_unassign_without_worker_time_allowed(self):
        ...
        task = Task.objects.create(
            name='Assigned, no estimate', job=self.job, service_price_id=1,
        )
    ...
    def test_unassign_task(self):
        task = Task.objects.create(
            ...
            assignee=self.user, worker_queue=1, service_price_id=1,
            est_worker_time=timedelta(hours=1),
        )
        response = self.client.post(
            f'/api/tasks/{task.pk}/assign/',
    ...
    def test_assign_requires_permission(self):
        viewer = User.objects.create_user(username='viewer', password='testpass')
        self.client.login(username='viewer', password='testpass')
        task = Task.objects.create(name='Task', job=self.job, service_price_id=1)
        response = self.client.post(
            f'/api/tasks/{task.pk}/assign/',
            data={'assignee': self.user.pk, 'worker_queue': 1},
            content_type='application/json',
