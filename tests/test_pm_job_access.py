...
class PlanTaskPMAccessTest(BaseTestCase):
    def setUp(self):
        ...
        self.scheme = ServicePrice.objects.create(
            name='Hourly PM PT', algorithm=ServicePrice.ENTERED_QTY,
...
class TaskAndContactGuardTest(BaseTestCase):
    def setUp(self):
        ...
        # Task.service_price is NOT NULL at the DB level; supply one.
        self.cat = AccountingCategory.objects.create(code='LAB-tk', name='Labor TK')
        self.scheme = ServicePrice.objects.create(
            name='Hourly TK', algorithm=ServicePrice.ENTERED_QTY,
            rate=Decimal('50.00'), unit_label='hour',
            accounting_category=self.cat,
        )
        self.task = Task.objects.create(
            job=self.job, name='Mill', service_price=self.scheme, sort_order=1,
        )
...
class TaskAssignPMAccessTest(BaseTestCase):
    ...
    def setUp(self):
        ...
        self.task = Task.objects.create(
            job=self.job, name='T', service_price=scheme, sort_order=1,
        )
