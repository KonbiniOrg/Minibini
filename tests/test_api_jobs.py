...
class JobTaskSubResourceTest(TestCase):
    ...
    def test_list_tasks_on_job(self):
        Task.objects.create(job=self.job, name='First task', service_price=self.scheme)
    ...
    def test_create_task_on_job(self):
        response = self.client.post(
            ...
            {'name': 'New task', 'service_price': self.scheme.pk},
            format='json',
        )
        ...
        # service_price set directly on Task (no TaskCharge)
        self.assertEqual(t.service_price_id, self.scheme.pk)

    def test_update_task_on_job(self):
        task = Task.objects.create(job=self.job, name='Original', service_price=self.scheme)
    ...
    def test_delete_task_on_job(self):
        task = Task.objects.create(job=self.job, name='Goner', service_price=self.scheme)
    ...
    def test_update_task_wrong_job_404(self):
        ...
        task = Task.objects.create(job=other_job, name='Theirs', service_price=self.scheme)
        response = self.client.patch(
            f'/api/jobs/{self.job.pk}/tasks/{task.pk}/',
            {'name': 'nope'},
            format='json',
    ...
    def test_create_task_any_authenticated(self):
        ...
        response = self.client.post(
            f'/api/jobs/{self.job.pk}/tasks/',
            {'name': 'Worker task', 'service_price': self.scheme.pk},
            format='json',
        )
        self.assertEqual(response.status_code, 201, response.data)
    ...
    def test_delete_task_with_bleps_blocked(self):
        ...
        task = Task.objects.create(job=self.job, name='Worked', service_price=self.scheme)
    ...
    def test_list_tasks_any_authenticated(self):
        Task.objects.create(job=self.job, name='Read me', service_price=self.scheme)
    ...
    def test_reorder_tasks_still_denied_for_worker(self):
        ...
        task = Task.objects.create(job=self.job, name='Reorder me', service_price=self.scheme)
...
class JobSerializerNestingTest(TestCase):
    ...
    def setUp(self):
        ...
        Task.objects.create(job=self.job, name='A task', service_price=self.scheme)
        Task.objects.create(job=self.job, name='B task', service_price=self.scheme)
...
class JobListQueryCountTest(TestCase):
    ...
    def _make_jobs(self, count):
        ...
        for i in range(existing, existing + count):
            ...
            Task.objects.create(job=job, name=f'Task A {i}', service_price=self.scheme)
            Task.objects.create(job=job, name=f'Task B {i}', service_price=self.scheme)
...
class JobPopulateFromTemplateTest(TestCase):
    ...
    def setUp(self):
        ...
        self.scheme = ServicePrice.objects.create(
            name='S-poptpl', algorithm=ServicePrice.FLAT_FEE,
...
class JobCopyFromWorksheetTest(TestCase):
    ...
    def setUp(self):
        ...
        self.scheme = ServicePrice.objects.create(
            name='S-cwaj', algorithm=ServicePrice.FLAT_FEE,
            rate=Decimal('1'), unit_label='ea', accounting_category=ac,
        )
...
class JobReorderTasksTest(TestCase):
    ...
    def setUp(self):
        ...
        self.scheme = ServicePrice.objects.create(
            name='S-reord', algorithm=ServicePrice.FLAT_FEE,
            rate=Decimal('1'), unit_label='ea', accounting_category=ac,
        )
        self.a = Task.objects.create(job=self.job, name='A', sort_order=0, service_price=self.scheme)
        self.b = Task.objects.create(job=self.job, name='B', sort_order=1, service_price=self.scheme)
        self.c = Task.objects.create(job=self.job, name='C', sort_order=2, service_price=self.scheme)
...
class JobPatchValidationErrorTest(TestCase):
    ...
    def test_patch_with_open_blep_returns_400(self):
        ...
        task = Task.objects.create(job=job, name='Active task', service_price=self.scheme)
...
class JobAddFromTemplateTest(TestCase):
    ...
    def setUp(self):
        ...
        self.scheme = ServicePrice.objects.create(
            name='S-aft', algorithm=ServicePrice.FLAT_FEE,
            rate=Decimal('1'), unit_label='ea', accounting_category=ac,
        )
...
class JobDetailInvoiceFieldTest(TestCase):
    ...
    def setUp(self):
        ...
        self.scheme = ServicePrice.objects.create(
            name='S-invf', algorithm=ServicePrice.FLAT_FEE,
            rate=Decimal('10.00'), unit_label='ea', accounting_category=ac,
        )
        self.task = Task.objects.create(
            job=self.job, name='Invoiceable Task', service_price=self.scheme,
        )
