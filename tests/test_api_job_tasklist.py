...
def _make_scheme(name_suffix=''):
    """Create a minimal flat-fee ServicePrice for tests that don't care about billing."""
    ...
    return ServicePrice.objects.create(
        name=f'S-tst-{name_suffix}', algorithm=ServicePrice.FLAT_FEE,
...
class MaterialCRUDTest(TestCase):
    ...
    def test_material_wrong_task(self):
        ...
        task2 = Task.objects.create(
            job=self.job, name='Other task', service_price=self.scheme,
        )
        response = self.client.patch(
...
class SubtaskCRUDTest(TestCase):
    ...
    def test_create_subtask(self):
        response = self.client.post(
            ...
            {'name': 'New subtask', 'est_qty': '3.00', 'service_price': self.scheme.pk},
            format='json',
        )
        self.assertEqual(response.status_code, 201)
    ...
    def test_create_subtask_any_authenticated_user(self):
        ...
        response = self.client.post(
            f'/api/tasks/{self.parent_task.pk}/subtasks/',
            {'name': 'Worker subtask', 'est_qty': '1.00', 'service_price': self.scheme.pk},
            format='json',
        )
        self.assertEqual(response.status_code, 201)
    ...
    def test_create_subtask_unauthenticated(self):
        ...
        response = self.client.post(
            f'/api/tasks/{self.parent_task.pk}/subtasks/',
            {'name': 'Fail', 'est_qty': '1.00', 'service_price': self.scheme.pk},
            format='json',
        )
        self.assertEqual(response.status_code, 403)
...
class TerminalTaskGuardTest(TestCase):
    ...
    def _make_task(self, task_status):
        return Task.objects.create(
            job=self.job, name='A task', status=task_status, service_price=self.scheme,
        )
...
class TaskSerializerFlattenTest(TestCase):
    ...
    def test_task_serializer_flattens_billing_fields(self):
        """Phase B: service_price, active_modifiers, est_qty, est_worker_time,
        ...
        self.assertEqual(row['service_price'], scheme.pk)
    ...
    def test_post_task_accepts_flat_billing_fields(self):
        """POST /api/jobs/<id>/tasks/ accepts service_price, active_modifiers,
        ...
        scheme = ServicePrice.objects.create(
            name='Hourly2', algorithm=ServicePrice.ELAPSED_TIME,
            rate=Decimal('50'), unit_label='hour',
            accounting_category=ac,
        )
        payload = {
...
class TaskListInvoiceFieldTest(TestCase):
    ...
    def setUp(self):
        ...
        self.task = Task.objects.create(
            job=self.job, name='Parent', service_price=self.scheme,
        )
        self.material = Material.objects.create(
        ...
        self.subtask = Task.objects.create(
            job=self.job, name='Child', service_price=self.scheme,
