...
def _pending_task(job, scheme):
    return Task.objects.create(
        job=job, name='Guard Task', service_price=scheme, status=Task.STATUS_PENDING,
    )


def _blocked_task(job, scheme):
    return Task.objects.create(
        job=job, name='Blocked Task', service_price=scheme, status=Task.STATUS_BLOCKED,
    )


def _in_progress_task(job, scheme):
    return Task.objects.create(
        job=job, name='In Progress Task', service_price=scheme,
