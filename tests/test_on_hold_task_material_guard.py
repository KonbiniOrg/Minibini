...
class OnHoldGuardBase(BaseTestCase):
    ...
    Fixture provides: a Contact, a ServicePrice with pk=1, a User 'admin',
    ...
    def setUp(self):
        ...
        self.scheme = (
            ServicePrice.objects.filter(algorithm=ServicePrice.FLAT_FEE).first()
            or ServicePrice.objects.first()
        )
...
class CompleteTaskOnHoldTest(OnHoldGuardBase):
    ...
    def setUp(self):
        ...
        flat_fee = ServicePrice.objects.filter(algorithm=ServicePrice.FLAT_FEE).first()
        if flat_fee is None:
            self.skipTest('No FLAT_FEE ServicePrice in fixture.')
...
class ReorderTasksOnHoldTest(OnHoldGuardBase):
    ...
    def test_reorder_tasks_blocked_on_on_hold_job(self):
        job = _on_hold_job(self.contact)
        t1 = Task.objects.create(job=job, name='T1', service_price=self.scheme, sort_order=1)
        t2 = Task.objects.create(job=job, name='T2', service_price=self.scheme, sort_order=2)
        with self.assertRaises(ValidationError) as ctx:
...
class CreateFromTemplateOnHoldTest(OnHoldGuardBase):
    ...
    def _get_flat_fee_template(self):
        ...
        scheme = ServicePrice.objects.filter(algorithm=ServicePrice.FLAT_FEE).first()
        if scheme is None:
            self.skipTest('No FLAT_FEE ServicePrice in fixture.')
