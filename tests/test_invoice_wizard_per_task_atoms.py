...
class WizardPerTaskAtomsTest(BaseTestCase):
    ...
    def setUp(self):
        ...
        from apps.jobs.models import ServicePrice, Job, Task, Blep
        ...
        self.task = Task.objects.create(job=self.job, name='Build-pta', service_price=self.scheme)
...
class WizardReadsTaskDirectlyTest(TestCase):
    ...
    task.service_price, not task.charge.*."""
    ...
    def setUp(self):
        ...
        self.scheme = ServicePrice.objects.create(
            name='Hourly', algorithm=ServicePrice.ENTERED_QTY,
            rate=Decimal('5.00'), unit_label='piece',
            accounting_category=ac,
        )
        self.task = Task.objects.create(
            job=self.job, name='Polish',
            service_price=self.scheme, active_modifiers=[],
