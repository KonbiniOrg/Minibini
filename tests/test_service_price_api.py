...
class ServicePriceAPITest(TestCase):
    def setUp(self):
        ...
        self.scheme = ServicePrice.objects.create(
            name='Hourly Labor', algorithm=ServicePrice.ELAPSED_TIME,
            rate=Decimal('45.00'), unit_label='hours',
            accounting_category=self.ac,
        )

    def test_list_requires_auth(self):
        resp = self.client.get('/api/service-prices/')
    ...
    def test_update(self):
        ...
        resp = self.client.patch(
            f'/api/service-prices/{self.scheme.pk}/',
    ...
    def test_delete(self):
        self.client.login(username='admin', password='testpass')
        resp = self.client.delete(f'/api/service-prices/{self.scheme.pk}/')
        self.assertEqual(resp.status_code, 200)
        self.assertFalse(ServicePrice.objects.filter(pk=self.scheme.pk).exists())

    def test_retrieve(self):
        self.client.login(username='worker', password='testpass')
        resp = self.client.get(f'/api/service-prices/{self.scheme.pk}/')
        self.assertEqual(resp.status_code, 200)
        self.assertEqual(resp.json()['name'], 'Hourly Labor')


class ServicePriceEditBlockTest(BaseTestCase):
    fixtures = []
...
class ServicePriceSupersedeEndpointTest(BaseTestCase):
    fixtures = []
    ...
    def test_supersede_creates_new_and_links_old(self):
        ...
        resp = self.client.post(
            f'/api/service-prices/{old.pk}/supersede/',
            {
        ...
        self.assertEqual(resp.status_code, 201)
        new_id = resp.json()['service_price_id']
        old.refresh_from_db()
    ...
    def test_supersede_requires_can_manage_config(self):
        ...
        resp = self.client.post(f'/api/service-prices/{old.pk}/supersede/', {})
        self.assertEqual(resp.status_code, 403)
...
class ServicePriceListFilterTest(BaseTestCase):
    fixtures = []
    ...
    def setUp(self):
        ...
        self.active = ServicePrice.objects.create(
            name='A-lf', algorithm='flat_fee', rate=Decimal('1'),
            unit_label='ea', accounting_category=self.ac,
        )
        self.old = ServicePrice.objects.create(
    ...
    def test_default_list_excludes_superseded(self):
        ...
        ids = [r['service_price_id'] for r in items]
    ...
    def test_include_superseded_returns_all(self):
        resp = self.client.get('/api/service-prices/?include_superseded=true')
        body = resp.json()
        items = body.get('results', body)
        ids = [r['service_price_id'] for r in items]
    ...
    def test_only_superseded_returns_just_old(self):
        resp = self.client.get('/api/service-prices/?only_superseded=true')
        body = resp.json()
        items = body.get('results', body)
        ids = [r['service_price_id'] for r in items]
        self.assertIn(self.old.pk, ids)
...
class ServicePriceSerializerExtraFieldsTest(BaseTestCase):
    fixtures = []
...
class ServicePriceSupersedeSameNameTest(BaseTestCase):
    """
    ...
    def test_supersede_with_same_name_as_old_succeeds(self):
        ...
        self.assertEqual(
            ServicePrice.objects.filter(name='SN-Hourly').count(), 1,
        )
        self.assertEqual(
            ServicePrice.objects.filter(name='SN-Hourly (v1)').count(), 1,
        )
