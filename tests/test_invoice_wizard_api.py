...
class InvoiceLineItemSerializerSourcesTest(TestCase):
    def setUp(self):
        ...
        self.scheme = ServicePrice.objects.create(
            name='Hourly-ils', algorithm=ServicePrice.ELAPSED_TIME,
    ...
    def test_task_source_renders_task_name_as_description(self):
        ...
        other_task = Task.objects.create(job=self.job, name='Cleanup', service_price=self.scheme)
...
class SourcePoolEndpointTest(TestCase):
    def setUp(self):
        ...
        self.job = Job.objects.create(contact=self.contact, status=Job.STATUS_APPROVED, job_number='JOB-2026-0001')
        self.scheme = ServicePrice.objects.create(
            name='Hourly-spe', algorithm=ServicePrice.ELAPSED_TIME,
            rate=Decimal('25.00'), unit_label='hours',
            accounting_category=self.category,
        )
        self.task = Task.objects.create(
...
class LineItemsFromAtomsEndpointTest(TestCase):
    def setUp(self):
        ...
        self.job = Job.objects.create(contact=self.contact, status=Job.STATUS_APPROVED, job_number='JOB-2026-0001')
        self.scheme = ServicePrice.objects.create(
            name='Hourly-lifa', algorithm=ServicePrice.ELAPSED_TIME,
            rate=Decimal('25.00'), unit_label='hours',
            accounting_category=self.category,
        )
        self.task = Task.objects.create(
...
class AddAtomsEndpointTest(TestCase):
    def setUp(self):
        ...
        self.job = Job.objects.create(contact=self.contact, status=Job.STATUS_APPROVED, job_number='JOB-2026-0001')
        self.scheme = ServicePrice.objects.create(
            name='Hourly-aae', algorithm=ServicePrice.ELAPSED_TIME,
            rate=Decimal('25.00'), unit_label='hours',
            accounting_category=self.category,
        )
...
class RemoveAtomsEndpointTest(TestCase):
    def setUp(self):
        ...
        self.job = Job.objects.create(contact=self.contact, status=Job.STATUS_APPROVED, job_number='JOB-2026-0001')
        self.scheme = ServicePrice.objects.create(
            name='Hourly-rae', algorithm=ServicePrice.ELAPSED_TIME,
            rate=Decimal('25.00'), unit_label='hours',
            accounting_category=self.category,
        )
        # task1 with a 2h blep — task atom = $50
