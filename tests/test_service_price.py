...
class ServicePriceModelTest(BaseTestCase):
    """Test creation of ServicePrice instances for all 3 algorithm types."""
    ...
    def test_create_elapsed_time_scheme(self):
        ...
        self.assertEqual(scheme.algorithm, ServicePrice.ELAPSED_TIME)
    ...
    def test_create_entered_qty_scheme_with_modifiers(self):
        ...
        self.assertEqual(scheme.algorithm, ServicePrice.ENTERED_QTY)
    ...
    def test_create_flat_fee_scheme(self):
        scheme = ServicePrice.objects.create(
            name='Setup Fee',
            algorithm=ServicePrice.FLAT_FEE,
            rate=Decimal('50.00'),
            unit_label='job',
            accounting_category=self.ac,
        )
        self.assertEqual(scheme.algorithm, ServicePrice.FLAT_FEE)
...
class ServicePriceComputeTest(BaseTestCase):
    """Test compute methods on ServicePrice."""
    ...
    def setUp(self):
        ...
        self.flat_scheme = ServicePrice.objects.create(
            name='Setup Fee',
            algorithm=ServicePrice.FLAT_FEE,
            rate=Decimal('50.00'),
            unit_label='job',
    ...
    def test_get_actual_qty_elapsed_time(self):
        ...
        labor_scheme = ServicePrice.objects.create(
            name='Labor Rate',
            algorithm=ServicePrice.ELAPSED_TIME,
            rate=Decimal('75.00'),
            unit_label='hour',
...
class TaskTemplateServicePriceTest(BaseTestCase):
    def setUp(self):
        super().setUp()
    ...
    def test_task_template_with_service_price(self):
        ...
        self.assertEqual(tmpl.service_price, self.scheme)
        self.assertEqual(tmpl.default_active_modifiers, ['messy'])
        self.assertEqual(tmpl.default_billable_qty, Decimal('4.00'))

    def test_task_template_without_service_price_rejected(self):
        """Phase B: service_price and default_billable_qty are NOT NULL."""
        from django.db import IntegrityError
        with self.assertRaises(IntegrityError):
            TaskTemplate.objects.create(template_name='Legacy Template')

    def test_task_template_api_includes_service_price(self):
        tmpl = TaskTemplate.objects.create(
            template_name='Assembly',
            service_price=self.scheme,
            default_active_modifiers=['messy'],
        ...
        data = resp.json()
        self.assertEqual(data['service_price'], self.scheme.pk)
        self.assertEqual(data['default_active_modifiers'], ['messy'])
        self.assertEqual(data['default_billable_qty'], '4.00')


class ServicePriceSupersessionFieldsTest(BaseTestCase):
...
class ServicePriceIsReferencedTest(BaseTestCase):
    ...
    def test_scheme_with_planTask_is_referenced(self):
        from apps.jobs.models import ServicePrice, PlanTask
        ...
        PlanTask.objects.create(
            est_worksheet=ws, name='t', service_price=s,
            est_qty=Decimal('1'),
        )
        self.assertTrue(s.is_referenced())

    def test_scheme_with_task_is_referenced(self):
        """Phase B: is_referenced checks Task.service_price, not TaskCharge."""
        from apps.jobs.models import ServicePrice, Task, Job
        from apps.contacts.models import Contact, Business
        ...
        Task.objects.create(job=job, name='t', service_price=s)
        self.assertTrue(s.is_referenced())
    ...
    def test_scheme_with_taskTemplate_is_referenced(self):
        ...
        TaskTemplate.objects.create(
            template_name='tt', service_price=s,
            default_billable_qty=Decimal('1'),
        )
        self.assertTrue(s.is_referenced())


class ServicePriceSupersedeMethodTest(BaseTestCase):
    fixtures = []
    ...
    def test_supersede_creates_new_scheme_and_links_old(self):
        ...
        old = ServicePrice.objects.create(
...
class ServicePriceVersionedSupersedeTest(BaseTestCase):
    fixtures = []
    ...
    def test_first_supersede_renames_old_to_v1_and_new_keeps_name(self):
        ...
        self.assertEqual(
            ServicePrice.objects.filter(name='Standard Labor').count(), 1,
        )
        self.assertEqual(
            ServicePrice.objects.filter(name='Standard Labor (v1)').count(), 1,
        )
    ...
    def test_supersede_does_not_share_modifiers_list_with_new_scheme(self):
        ...
        in ServicePriceSupersedeMethodTest, but worth keeping for the
        new code path.)
        """
        from apps.jobs.models import ServicePrice
        old = ServicePrice.objects.create(
...
class ServicePriceFreezeOnReferenceTest(BaseTestCase):
    fixtures = []
    ...
    def test_supersede_still_works_on_referenced_scheme(self):
        s = self._make_referenced_scheme()
        # Pass a new name to avoid the unique-name constraint on ServicePrice.name.
        new = s.supersede(name='S-frz-v2', rate=Decimal('99'))
        s.refresh_from_db()
        self.assertEqual(s.replaced_by, new)


class ServicePriceRequiresACTest(BaseTestCase):
    fixtures = []
...
class ServicePriceIsReferencedTaskTest(BaseTestCase):
    fixtures = []
    ...
    def test_is_referenced_counts_task_not_taskcharge(self):
        """After Phase B, ServicePrice.is_referenced checks Task instead of TaskCharge."""
        from apps.jobs.models import Task, ServicePrice, Job
        from apps.contacts.models import Contact, Business

        scheme = ServicePrice.objects.create(
            name='Test B2', algorithm=ServicePrice.FLAT_FEE,
        ...
        Task.objects.create(job=job, name='Direct', service_price=scheme)
