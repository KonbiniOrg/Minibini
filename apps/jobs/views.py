...
from .models import Job, Estimate, EstimateLineItem, Task, WorkOrder, WorkOrderTemplate, TaskTemplate, EstWorksheet, TaskMapping, TaskInstanceMapping, BundlingRule
...
def bundling_rule_list(request):
    """List all BundlingRules."""
    show_all = request.GET.get('show_all', '0') == '1'

    if show_all:
        rules = BundlingRule.objects.all()
    else:
        rules = BundlingRule.objects.filter(is_active=True)
...
def bundling_rule_detail(request, pk):
    """Display BundlingRule details."""
    rule = get_object_or_404(BundlingRule, pk=pk)
...
def bundling_rule_edit(request, pk):
    """Edit an existing BundlingRule."""
    from .forms import BundlingRuleForm

    rule = get_object_or_404(BundlingRule, pk=pk)

    if request.method == 'POST':
        form = BundlingRuleForm(request.POST, instance=rule)
        if form.is_valid():
            form.save()
        ...
        form = BundlingRuleForm(instance=rule)
