...
**Backend:** `InvoiceViewSet.get_queryset` is annotated to compute totals and derived fields; `InvoiceSummarySerializer` (a lightweight read-only serializer) is used for the list response. List read permission stays `IsAuthenticated` — the Financials sidebar gate is a UI convention only; authenticated users with the direct URL can also reach the API.
