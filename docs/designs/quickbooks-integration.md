...
1. **Gate** — `_assert_all_lines_categorized` raises before any external call if any line lacks an accounting category. In normal operation this never fires: invoice compose (task-owned-money Phase 3) already stamps the configured `fallback_accounting_category` onto any line whose source atom has no AC of its own — see `invoicing-and-expenses.md` §"Fallback accounting category" and `data-constraints.md` §1.1. Not to be confused with `AccountingCategory.qbo_item_id`, an unrelated, pre-existing per-category *ItemRef* fallback used by `_resolve_item_ref` below — same word, different mechanism (one substitutes a QBO Item when the line has no catalog identity; the other substitutes an accounting category when the source atom has none).
