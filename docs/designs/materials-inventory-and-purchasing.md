...
The PO detail page (`PurchaseOrderDetail.svelte`) shows `billed_total`, `po_total`, and an `is_fully_billed` badge sourced from `PurchaseOrderSerializer`. A Bills panel lists linked Bills (via `?purchase_order=<id>`).
