...
| `/api/rate-schemes/` | `IsAuthenticated` | `can_manage_config` | `retire` / `reactivate` actions also `can_manage_config`. Any field is directly editable, referenced or not — no frozen-fields rejection (task-owned-money Phase 1). |
...
- **`claim_estimate_line` presence-gate (mint-by-modal, estimating-structure spec §2/§4)** — the same money-field presence pattern, applied to a non-money field: `POST /api/jobs/{id}/tasks/` and `POST /api/jobs/{id}/add-from-template/` both accept an optional `claim_estimate_line` body key that binds the just-created Task to an accepted estimate's line as its mint claim (`MintService.claim_atom_for_line`). The key's **mere presence**, not its value, triggers the gate — checked *before* serializer validation (fail-closed): a non-manager, non-PM caller including the key gets 403 even if the rest of the body is otherwise invalid or the key would self-serve-create fine without it. Gate is `CanManageJobOrPM` (`request.user.has_perm('core.can_manage_jobs')` or `JobService.user_can_manage`), same predicate as the task-money gate above but a distinct check — a worker with no atom and no PM standing can still self-serve plain task creation on the same endpoint, just never with a claim attached. A non-numeric or missing-on-this-job line id 400s (`{'detail': ...}`) rather than 403ing — those are input errors, not permission ones. Implemented twice by design (`apps/api/mixins.py` `JobTaskMixin.tasks`, `apps/api/jobs/views.py` `_resolve_claim_line` for `add_from_template`) — mixins.py can't import from jobs/views cleanly, so the two copies are kept in sync by hand; see either docstring.
- **Task money-field writes (task-owned-money Phase 1)** — layered *inside* the endpoint-open task-add/edit rule above: `TaskSerializer.MONEY_FIELDS` (`rate`, `unit_label`, `qty_source`, `accounting_category`, `active_modifiers`, `source_scheme`) can only be set by `CanManageJobOrPM` (the `can_manage_jobs` atom or the task's job's PM) or the `can_manage_financials` atom. The gate triggers on **presence of the key in the raw request body**, not its value — a worker posting `active_modifiers: []` still 403s, because presence alone means "override the preset's money". Everyone else gets stamp-only creation (`rate_scheme` id — the preset to stamp from — with no money-field overrides) and non-money edits. `rate_scheme` itself isn't a `MONEY_FIELD`: any authenticated user may pick a preset to stamp; they just can't override what it stamps. See `architecture-and-conventions.md` §3.5. **`source_scheme` joined `MONEY_FIELDS` for RM browser-testing note 5** (client-side restamp) — writable on UPDATE only (create still 400s it; `rate_scheme` remains create's only stamping trigger), gated identically. See `estimates-and-prices.md` §3.6a/§10.1 and `data-constraints.md` §1.11. **`can_write_money`, a read-only `SerializerMethodField` (RM browser-testing note 6)** exposes this exact predicate (`TaskSerializer._can_write_money()`) on every task payload — the SPA's edit-task money-field gating (`WorkItemForm.svelte`) reads it directly instead of the job-management-only `can_manage` field, so a `can_manage_financials`-only caller (no atom, not the PM) doesn't see money fields disabled/greyed that the server would in fact accept. See `estimates-and-prices.md` §3.6b.
...
- **`POST /api/jobs/{id}/add-from-template/`** and **`POST /api/jobs/{id}/create_material/`** are `IsAuthenticated` only — workers can self-serve adding template-driven tasks and materials. `add-from-template` layers the identical money-field gate onto its one overridable money-affecting field: a request that includes the `active_modifiers` key (even `[]`, overriding the `ServiceItem`'s own `default_active_modifiers`) requires `CanManageJobOrPM` or `can_manage_financials`; an omitted key rides the template's defaults through unchanged, open to anyone.
...
- **`work_declined` PATCH on an estimate line (acceptance checklist, estimating-structure spec)** — `PATCH /api/estimates/{id}/line-items/{item_id}/` gates on the usual `CanManageJobOrPM` for the `line_item_detail` action, same as every other line-item write. The **carve-out is in the service, not the permission class**: every other field on `EstimateLineItem` is draft-only (`EstimateService.update_line_item` 400s a PATCH on a non-draft estimate before touching fields), but a body containing *only* `{'work_declined': ...}` is routed to `EstimateService._set_work_declined` instead, which requires the estimate to be **accepted** (draft and open both refuse) and refuses on atom-backed, adjustment, deposit, or catalog-identity lines. A body mixing `work_declined` with any other key is rejected outright regardless of estimate status, so the two update paths — normal draft-only field edits and the accepted-only decline toggle — can never blur into one PATCH.
- **Manual `approved → in_progress` is retired** — `PATCH /api/jobs/{id}/` can no longer drive a job onto the floor directly; `JobService.update_job` raises unless the transition is a `system_transition` (auto-release firing once the accepted estimate's checklist is fully answered, or another system-driven walk — timeslip-start, work-complete's intermediate hop, invoice-completion cascade). The status pill (`can_manage`-gated, same as any other status edit) no longer offers the transition at all — see `jobs-and-tasks.md` §3.3 and §9.1.
