...
5. **Carry-over "Phase B" — DECIDED 2026-06-02: leave as-is.** Phase B is **not** gated on "no worksheet"; it runs every accept and is scoped **per line** by `not li.sources.exists()` — it materializes a Job Task/Material only for line items the worksheet didn't already cover (template/PLI refs with no worksheet source). So a pure-worksheet estimate triggers nothing in Phase B, a worksheet-less estimate triggers it for everything, and a mixed estimate gets the hand-added lines filled in. Operator chose to **keep the current per-line behavior unchanged** for now and may later revisit whether to make it a strict "only when the job has no worksheet" gate (which would drop hand-added catalog lines on a worksheet-backed estimate). No code change in this redesign.
