...
describe('EstimateAddLineForm', () => {
  ...
  describe('kind=material', () => {
    it('prefills description from typed and posts freeform_kind material with the default AC', async () => {
      ...
      await fireEvent.click(getByRole('button', { name: /add/i }));
      expect(api.post).toHaveBeenCalledWith('/api/estimates/42/line-items/',
        expect.objectContaining({ freeform_kind: 'material', accounting_category: 7, price: '30' }));
    ...
    it('does not block save when no default AC is configured (backend fills it)', async () => {
      ...
      await fireEvent.click(getByRole('button', { name: /add/i }));
      expect(api.post).toHaveBeenCalledWith('/api/estimates/42/line-items/',
        expect.objectContaining({ freeform_kind: 'material' }));
    });
  ...
  describe('kind=fee', () => {
    it('defaults qty to 1, prefills description, requires AC, and posts freeform_kind fee', async () => {
      ...
      expect(api.post).toHaveBeenCalledWith('/api/estimates/42/line-items/',
        expect.objectContaining({
          description: 'Rush charge', freeform_kind: 'fee', accounting_category: 7, price: '50',
        }));
      expect(onSaved).toHaveBeenCalled();
    });
  ...
  describe('kind=work', () => {
    it('loads task-applicable rate schemes and offers them as a preset dropdown', async () => {
      ...
      const { findByRole } = render(EstimateAddLineForm, {
        props: { open: true, choice, estimateId: 42, categories: cats, onSaved: vi.fn() },
      });
    ...
    it('preselects the configured default preset when it is in the list, and stamps its fields', async () => {
      ...
      const choice = { type: 'freeform', kind: 'work', typed: 'Custom milling' };
      const { findByRole, getByLabelText } = render(EstimateAddLineForm, {
        props: { open: true, choice, estimateId: 42, categories: cats, onSaved: vi.fn() },
      });
      // Wait for both async loads (schemes, then settings) to settle before
      // reading the preselected value.
    ...
    it('picking a preset stamps rate/units/AC into editable fields, then submits plain values (no scheme id)', async () => {
      const choice = { type: 'freeform', kind: 'work', typed: 'Custom milling' };
      const onSaved = vi.fn();
      const { findByRole, getByLabelText, getByRole } = render(EstimateAddLineForm, {
        props: { open: true, choice, estimateId: 42, categories: cats, onSaved },
      });
      const select = await findByRole('combobox', { name: /preset/i });
      ...
      await fireEvent.click(getByRole('button', { name: /add/i }));
      expect(api.post).toHaveBeenCalledWith('/api/estimates/42/line-items/', {
        description: 'Custom milling', qty: '4', units: 'hour', price: '80',
