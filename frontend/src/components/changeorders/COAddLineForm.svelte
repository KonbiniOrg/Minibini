<script>
  ...
  async function save() {
    ...
    if (choice.type === 'service') {
      ...
      // EstimateService._validate_qty (shared with ChangeOrderService's
      // add_line_item). A blank qty falls back to '0' in the payload below,
      // same as the zero-price case above.
