  ...
  async function actuallySave(propagate) {
    ...
    try {
      if (mode === 'edit' && material) {
        ...
        if (planTaskId) {
          await api.patch(`/api/plan-tasks/${planTaskId}/materials/${material.plan_material_id}/`, patch);
        } else {
          await api.patch(`/api/est-worksheets/${worksheetId}/plan-materials/${material.plan_material_id}/`, patch);
        }
      } else if (planTaskId) {
        await api.post(`/api/plan-tasks/${planTaskId}/materials/`, fullPayload);
      } else {
        await api.post(`/api/est-worksheets/${worksheetId}/plan-materials/`, fullPayload);
      }
      onSaved();
    } catch (e) {
